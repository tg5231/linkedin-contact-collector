(function () {
  ...
  function renderRestNotice(done, total, restMinutes = DEFAULT_REST_MINUTES) {
    ...
    renderLogPanel("LinkedIn Contact Collector", [
      ...
      `已连续采集：${done} / ${total}`,
      "为了降低触发风控的概率，已自动暂停。",
      `建议休息 ${restMinutes} 分钟后，再从插件弹窗点击“继续”。`
    ]);
  }
  ...
  function collectVisibleProfileUrls(existing) {
    ...
    for (const a of Array.from(document.querySelectorAll('a[href*="/in/"]'))) {
      ...
      if (!href || !isLinkedInProfileUrl(href) || seen.has(href)) continue;
  ...
  function extractConnectionDate(text) {
    ...
    const zh = raw.match(/于\s*(\d{4})年(\d{1,2})月(\d{1,2})日成为好友/);
    if (zh) {
      const y = zh[1];
  ...
  async function startFromConnectionsList() {
    ...
    for (let round = 0; round < 80; round += 1) {
      ...
      items = collectVisibleProfileUrls(items);
      if (status) status.textContent = `正在采集好友链接：${items.length} 个；本次最多 ${batchLimit} 个；滚动轮次 ${round + 1}`;
      if (items.length >= batchLimit) break;

      if (items.length === lastCount) stableRounds += 1;
      else stableRounds = 0;
      lastCount = items.length;
      if (stableRounds >= 5 && items.length > 0) break;
    ...
    if (!items.length) {
      if (status) status.textContent = "没有采集到好友主页链接，请确认当前是 LinkedIn 好友列表页。";
  ...
  async function goNextIfEnabled(expectedIndex) {
    const { queue = [], index = 0, running = false, delayMs = NEXT_PROFILE_DELAY_MS, restAfter = DEFAULT_REST_AFTER, restMinutes = DEFAULT_REST_MINUTES } = await storageGet(["queue", "index", "running", "delayMs", "restAfter", "restMinutes"]);
    if (!running) return;
    ...
    if (Number(restAfter) > 0 && nextIndex >= Number(restAfter)) {
      await storageSet({ running: false });
      renderRestNotice(nextIndex, queue.length, Number(restMinutes) || DEFAULT_REST_MINUTES);
      return;
    }
